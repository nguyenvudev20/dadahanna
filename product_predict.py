...
uploaded_file = st.file_uploader("📥 Tải lên file Excel (data_product.xlsx)", type=["xlsx"])
...
if uploaded_file:
    ...
    forecast_months = st.slider("🔮 Số tháng cần dự báo", min_value=1, max_value=12, value=3)

    df_product = df[df['code'] == selected_code]
    df_monthly = df_product.resample('ME', on='date')['numsell'].sum().fillna(0).reset_index()
    ...
    df_monthly['is_holiday'] = df_monthly['month'].isin([1, 2, 4, 9, 12]).astype(int)
    ...
    if len(df_monthly) < 10:
        ...
        features = ['numsell', 'month', 'quarter', 'year', 'days_in_month', 'is_holiday']
        scaler = MinMaxScaler()
        ...
        model.fit(X_train, y_train, epochs=200, batch_size=8, validation_data=(X_val, y_val), verbose=0)
        ...
        for i in range(forecast_months):
            ...
            input_scaled = scaler.transform([[0, month, quarter, year, days_in_month, is_holiday]])
            ...
            next_scaled = np.concatenate([[pred_scaled[0][0]], input_time_features])
        ...
        fig.add_trace(go.Scatter(x=real_dates,
                                 y=scaler.inverse_transform(np.hstack([y_val.reshape(-1,1), np.zeros((len(y_val), len(features)-1))]))[:,0],
                                 mode='lines+markers', name='Giá trị thực'))

        fig.add_trace(go.Scatter(x=real_dates,
                                 y=scaler.inverse_transform(np.hstack([y_pred, np.zeros((len(y_pred), len(features)-1))]))[:,0],
        ...
        fig.update_layout(title="📊 So sánh giá trị thực và dự báo", xaxis_title="Thời gian", yaxis_title="Số lượng")
        ...
        df_all_monthly['month'] = df_all_monthly['date'].dt.to_period('M').dt.to_timestamp('M')
        top_agg = df_all_monthly.groupby(['month', 'code'])['numsell'].sum().reset_index()
        top_codes = top_agg.groupby('code')['numsell'].sum().nlargest(30).index.tolist()

        future_top_preds = []
        for code in top_codes:
            sub_df = df[df['code'] == code]
            sub_monthly = sub_df.resample('ME', on='date')['numsell'].sum().fillna(0).reset_index()
            if len(sub_monthly) >= seq_len + forecast_months:
                ...
                sub_monthly['is_holiday'] = sub_monthly['month'].isin([1, 2]).astype(int)
                ...
                if len(sub_scaled) >= seq_len:
                    ...
                    forecast_date = sub_monthly['date'].max() + pd.DateOffset(months=1)
        ...
        top_demand_df = pd.DataFrame(future_top_preds, columns=['Mã sản phẩm', 'Dự báo tháng tới'])
        top_demand_df = top_demand_df.sort_values(by='Dự báo tháng tới', ascending=False)
